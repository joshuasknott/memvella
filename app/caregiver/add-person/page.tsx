...
import { ArrowLeft, Camera, Check, Plus, Sparkles, Lightbulb, UserCheck } from 'lucide-react';
...
export default function AddPersonPage() {
  ...
  return (
    <>
      {/* Top App Bar */}
      <header className="fixed top-0 w-full max-w-md z-50 bg-white/80 backdrop-blur-md flex items-center px-6 py-4 shadow-sm shadow-primary/5">
        <div className="flex items-center gap-4 w-full">
          <Link href="/caregiver" className="active:scale-95 duration-150 hover:opacity-80 transition-opacity text-primary">
            <ArrowLeft className="w-6 h-6" />
          </Link>
          <h1 className="font-headline font-bold text-xl text-primary tracking-tight">Add to Family</h1>
          {/* MANDATORY FIX: Removed Redundant 'Save' text link from here */}
        </div>
      </header>

      <main className="pt-24 px-6 space-y-10 pb-32">
        {/* Photo Upload Hero */}
      ...
      <footer className="fixed bottom-0 w-full max-w-md px-6 pb-8 pt-4 bg-linear-to-t from-surface via-surface/90 to-transparent z-40">
        <button 
